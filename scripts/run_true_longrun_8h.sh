#!/usr/bin/env bash
# R7.0-Ops-H: guarded native true long-run (8h min-runtime, visible heartbeat, no auto-merge).
set -euo pipefail
...
LOG_DIR="${ROOT}/outputs/operator/true_longrun_8h/${RUN_ID}"
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_DIR}/run.log"

echo "=== true-longrun-8h start (${RUN_ID}) ===" | tee "${LOG_FILE}"
...
if command -v caffeinate >/dev/null 2>&1; then
  ...
  echo "true-longrun-8h: WARN: caffeinate not found; continuing without sleep guard" | tee -a "${LOG_FILE}"
  "${DEV_LOOP_CMD[@]}" 2>&1 | tee -a "${LOG_FILE}"
fi
dev_loop_rc=${PIPESTATUS[0]}
set -e

echo "=== true-longrun-8h end (dev_loop_rc=${dev_loop_rc}) log=${LOG_FILE} ===" | tee -a "${LOG_FILE}"
...
if [[ -n "${latest_evidence}" ]]; then
  echo "true-longrun-8h: latest evidence=${latest_evidence}" | tee -a "${LOG_FILE}"
fi
