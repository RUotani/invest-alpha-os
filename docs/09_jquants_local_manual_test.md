...
- **コマンド（概念）**: `debug jquants-daily-quotes --code 7974 --date 2024-02-18 --live`（BASE URL・API Key は **各自の `.env` のみ**。日付は **契約ウィンドウ内**にすること）
- **結果の要約**: `status: success`, `code: 7974`, `date: 2024-02-18`, `row_count: 1`, `source_key: data`

**契約ウィンドウ（実運用の目安・Task 9.2 / Ops ワークフローの例）**：**`watchlist bars` を `from-date`〜`to-date` でまとめ取りする場合**、手元環境では **`2024-02-18`〜`2026-02-17`**（両端込み）から始めると安全でした。**`FROM=2024-02-17` では一部銘柄で HTTP 400**（データ提供開始に関するメッセージが返る事例あり）。ダッシュボードの文言が **`2024-02-17` 開始**でも、**ガード値と Makefile 例は `2024-02-18`** に合わせ、**自分のプランに合わせて `.env` の `JQUANTS_DATA_AVAILABLE_*` を必ず上書き**すること。

**`2024-02-17`（注記）**：ウィンドウ例の開始より **1 日早く**、銘柄によっては **`http_error`/400** になり得ます（bulk は **`2024-02-18` 起点を推奨**）。

**`2024-02-16`（注記）**：**`2024-02-18` 開始のガード**に合わせた場合 **`date_out_of_available_range`** になり得る。

**契約データ範囲外**の典型（API が返し得る趣旨: *Your subscription covers the following dates: …*）:

- **`2024-01-04`** — 契約開始 **前**
- **`2024-02-16`** — 開始日前（ガード開始が `2024-02-18` のとき **範囲外**）
...
- **契約（API が返した趣旨・要約）**: *Your subscription covers the following dates: 2024-02-17 ~ 2026-02-17.*（文言は環境により異なる）／**リポの `.env.example` と `make jq-*` の例・CI 既定は、`bars` まとめ取りで観測された境界に合わせ開始を `2024-02-18` とする**。
- **`2024-02-16`〜`2024-02-17`（記録ベースの注記）**：ガード開始を `2024-02-18` にした場合、`2024-02-16` は **範囲外**。`2024-02-17` は環境により **400 / 範囲外**になり得る。
