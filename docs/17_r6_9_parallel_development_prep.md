...
## 0. Stage 3 運用（R6.9-B 注釈）

本ドキュメントや関連タスクで言及する **「Stage 3」** は、**Claude Code** と **Cursor Composer2** を **同時に起動して並行作業する**運用を指さない。

- **Cursor Composer2 を単独**で用い、**1 回の Markdown 指示**に **確認・ドキュメント更新・テスト・安全確認・作業ブランチの push・branch CI（`tests` ワークフロー）の結果確認**まで寄せる。
- **人間**が毎回ターミナルで `pytest` / `grep` / `git status` / `gh run` を細かく打つ運用には**戻さない**（エージェント側で自走する前提）。
- **`main`** への取り込みは **原則 ChatGPT 判断後**。その間は **`main` 未反映の docs-only ブランチ**（例: `work/r6-9-b-stage3-workflow-docs`）で整理してよい。

下記の **`git worktree`（同一リポジトリから別作業ディレクトリを作る仕組み）** は **作業ディレクトリの分離**のための道具であり、**複数 AI の同時並行**とは切り離して読むこと。
