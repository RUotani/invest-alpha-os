...
**ステータス**: 作業ブランチ `work/r6-14-e-approved-single-worktree-cleanup` のみ（**`main` 未反映**）。**本記録は承認済み 1 worktree の `git worktree remove` のみ** — **`rm -rf`・local/remote branch 削除・他 path の削除は行っていない**。
...
- **R6.14-F**: 次の **単一 cleanup** または **branch cleanup proposal**（**別承認** · **1 本ずつ**）。
