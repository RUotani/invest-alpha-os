...
> **固定入口は** [`latest/README_FOR_USER.md`](../latest/README_FOR_USER.md) **（初日運用はそちらを優先）**  
> **自動売買ではありません。** 候補発掘・監視・**深掘り候補**のレポートです。  
...
| 役割 | 銘柄 | 要点 |
| --- | --- | --- |
| **第1深掘り候補** | 285A（キオクシア） | メモリ/NANDテーマ・モメンタム強いが **短期過熱**（20日 +73%）— **売買指示ではない** |
| **深掘り候補** | AAPL（Apple） | 52週高値近辺での反応 — **調査用** |
| **深掘り候補** | QQQ（Nasdaq 100 ETF） | 指数モメンタムの観測（リスクオン proxy）— **調査用** |
