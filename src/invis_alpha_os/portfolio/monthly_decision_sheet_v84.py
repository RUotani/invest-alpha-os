...
def build_monthly_decision_sheet_v84_markdown(
    *,
    ...
    lines = [
        ...
        "",
        "理由:",
        f"- 現金比率が{_fmt_pct_1(cash_pct)}で、最低目安15%を下回っている",
        f"- 個別株比率が{_fmt_pct_1(individual_pct)}で、目安10〜15%を上回っている",
        "- データ品質・guardrail未達下では、強い新規リスク候補として扱えない",
        "",
        "今月やること:",
        "1. 現金回復と個別株比率の整理候補を優先確認",
        "2. 既存ポジションの重複リスク・高ボラ偏りを確認",
        "3. 次月runでデータ品質と候補抽出が改善するかを見る",
        "",
        "今月やらないこと:",
        "- 根拠不足の個別株・高ベータ銘柄を追加しない",
        "- 現金不足を無視して新規リスクを増やさない",
