...
- **`debug jquants-watchlist-bars`**（Task 6）: `jp_watchlist` を **dry-run / `--preview-request` / `--live`** で一括確認。**`--save-summary`**（Task 9 / **9.1**）で **sanitized JSON**（**`dry_run_count`・`preview_count`** ほかカウンタ分離）を **`outputs/jquants_smoke/`** にローカル保存（Git 対象外）。**4 桁数字のみ wire**、`285A` 等は **skip**。**Task 9.2**：人間環境で **watchlist limit 3 live + `--save-summary` 成功記録**あり（`7011`/`6501`/`6506`、`2024-02-19`）；**`latest.json`** は同ディレクトリで **`.gitignore`**。**Task 10**：**`daily` がこの `latest.json` を読むだけ**（下の **`alpha-os daily`** 節と同様 **`daily` で live はしない**）。
