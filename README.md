...
- **`debug jquants-watchlist-bars`**（Task 6）: `jp_watchlist` を **dry-run / `--preview-request` / `--live`** で一括確認（**`--date` または `--from-date`/`--to-date` 必須**、**`--limit`**）。**4 桁数字のみ wire**、`285A` 等は **skip**。**daily レポート統合は Task 7**。
