...
- **`debug jquants-watchlist-bars`**（Task 6）: `jp_watchlist` を **dry-run / `--preview-request` / `--live`** で一括確認。**`jp_watchlist` からの wire** は **ASCII 英数字 4 文字**（例：**`7011`・`285A`**。**大文字へ正規化**。記号や 5 字超などは **`skipped_unsupported_code`**）。**`--save-summary`**（Task 9 / **9.1**）で **sanitized JSON**を **`outputs/jquants_smoke/`** にローカル保存（Git 対象外）。**Task 9.2**：人間環境で **watchlist limit 3 live + `--save-summary` 成功記録**あり（`7011`/`6501`/`6506`、`2024-02-19`）；**`latest.json`** は同ディレクトリで **`.gitignore`**。**Task 10**：**`daily` がこの `latest.json` を読むだけ**（下の **`alpha-os daily`** 節と同様 **`daily` で live はしない**）。
- **`alpha-os daily`**（Task 7–8）: レポートに **J-Quants Watchlist Bars Check** と **readiness（Green / Yellow / Red）**。**HTTP なし**の **dry-run 集計のみ**。**Task 10**：ローカルの **`outputs/jquants_smoke/latest.json`**（**`--save-summary` で生成した sanitized**）を **読むだけ**して **Latest local smoke summary** を表示。**live ではなく保存ファイル参照**。**`make verify` / CI も同様に HTTP なし**。
