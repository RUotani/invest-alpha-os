...
- **`debug jquants-watchlist-bars`**（Task 6）: `jp_watchlist` を **dry-run / `--preview-request` / `--live`** で一括確認。**`--save-summary`**（Task 9 / **9.1**）で **sanitized JSON**（**`dry_run_count`・`preview_count`** ほかカウンタ分離）を **`outputs/jquants_smoke/`** にローカル保存（Git 対象外）。**4 桁数字のみ wire**、`285A` 等は **skip**。
