...
def test_live_get_url_omits_placeholder_apikey_when_stooq_env_unset(mock_urlopen: MagicMock) -> None:
    ...
    low = url.lower()
    assert "redacted_required_later" not in low
    assert "<redacted" not in url
    qdict = dict(parse_qsl(urlparse(url).query))
    assert "apikey" not in qdict


@patch.object(uplp, "urlopen")
def test_live_get_includes_stooq_key_in_transport_only_secret_not_echoed(mock_urlopen: MagicMock) -> None:
    secret = "st_integration_secret_value_please_never_commit"
    _patch_urlopen_ok(mock_urlopen, _csv_body())
    with patch.dict(
        os.environ,
        {uplp.CONFIRM_US_LIVE_HTTP_ENV: "YES", uplp.STOOQ_APIKEY_ENV: secret},
        clear=False,
    ):
        r = runner.invoke(
            app,
    ...
    req = mock_urlopen.call_args[0][0]
    transport = getattr(req, "full_url", "") or ""
    assert secret in transport
    qdict = dict(parse_qsl(urlparse(transport).query))
    assert qdict.get("apikey") == secret


@patch.object(uplp, "urlopen")
def test_live_provider_api_key_required_exit_two(mock_urlopen: MagicMock) -> None:
    body = "Please get your apikey before downloading historical data.\n"
    _patch_urlopen_ok(mock_urlopen, body.encode("utf-8"))
    with patch.dict(os.environ, {uplp.CONFIRM_US_LIVE_HTTP_ENV: "YES"}, clear=False):
        r = runner.invoke(
            app,
    ...
    assert p["reason"] == "provider_api_key_required"
    assert p["http_status"] == 200
    assert p["raw_response_included"] is False
    assert p["response_diagnostics"]["body_kind"] == "api_key_required"
    dumped = json.dumps(p).lower()
    assert "downloading historical" not in dumped
