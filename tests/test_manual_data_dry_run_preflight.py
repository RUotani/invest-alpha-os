...
def test_preflight_without_input_file(tmp_path: Path) -> None:
    ...
    assert result.json_payload["actual_import_prohibited"] is True
    assert result.json_payload["import_flow_dry_run"] is None
    assert "285A" in ",".join(result.json_payload["required_targets"])
    assert "manual_jp_bars.csv" in result.json_payload["expected_filenames"]
    assert "--execute-import false" in result.json_payload["dry_run_command"]
    assert "ticker,date,open,high,low,close,volume" in ",".join(result.json_payload["required_columns"])
